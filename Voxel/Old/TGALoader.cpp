...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{
		std::cout<<"Could not open texture file\n";	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		std::cout<<"Could not read file header\n";		// If it fails, display an error message 
	...
	{
		std::cout<<"TGA file be type 2 or type 10 \n";	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{										
		std::cout<< "Could not read info header\n";		// Display error
	...
	{
		std::cout<<"Invalid texture information\n";	// Display Error
	...
	{
		std::cout<<"Could not allocate memory for image\n";	// Display Error
	...
	{
		std::cout<<"Could not read image data\n";		// Display Error
...
{ 
	...
	do
	{
		...
		{
			std::cout<<"Could not read RLE header\n";	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
