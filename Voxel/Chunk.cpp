...
void Chunk::SetFaces( bool* faces, int i, int x, int y, int z)
{
	...
	//Left & Right
	//if(x -1 < 0 || m_tree->GetBlock(x - 1, y, z) == 0)
	//{faces[0] = true;}
	//else{faces[0] = false;}
	//if(x + 1 >= CHUNK_SIZE || m_tree->GetBlock(x + 1, y, z) == 0)
	//{faces[1] = true;}
	//else{faces[1] = false;}

	////Top & Bottom
	//if(y - 1 < 0 || m_tree->GetBlock(x, y - 1, z) == 0)
	//{faces[2] = true;}
	//else{faces[2] = false;}
	//if(y + 1 >= CHUNK_SIZE || m_tree->GetBlock(x, y + 1, z) == 0)
	//{faces[3] = true;}
	//else{faces[3] = false;}

	////Front & Back
	//if(z - 1 < 0 || m_tree->GetBlock(x, y, z - 1) == 0)
	//{faces[4] = true;}
	//else{faces[4] = false;}
	//if(z + 1 >= CHUNK_SIZE || m_tree->GetBlock(x, y, z + 1) == 0)
	//{faces[5] = true;}
	//else{faces[5] = false;}
}
...
void Chunk::CreateVBO(TextureAtlas* texture )
{
	...
	//{
	//	int x = i % CHUNK_SIZE;
	//	int rows = (i - x) / CHUNK_SIZE;
	//	int y = (int)(rows % CHUNK_SIZE);
	//	int z = (rows / CHUNK_SIZE);

	//	if(m_blocks[i].m_type != 0)
	//	{
	//		bool faces[6];

	//		SetFaces(faces, i, x, y, z);
	//		m_texture->SetTexture(m_blocks[i].m_type);
	//		CreateBlock(faces, x * Block::BLOCK_SIZE, y * Block::BLOCK_SIZE, z * Block::BLOCK_SIZE);
	//	}
	//	std::cout << m_blocks[i].m_type;
	//}
	BlockNode* node = m_tree->GetNode();
